...
- `-d, --delta_t`: The size of each timestep (the unit depends on the simulation). Default value: 0.014
- `-e, --t_end`: Time at which to stop the simulation (the unit depends on the simulation). Default value: 1000 
...
- `-b, --bench`: Activate benchmarking: the execution time of 10 iterations of the program will be averaged (logging turned off for maximum performance)
