...
#pragma omp parallel for num_threads(numThreads)
    for (size_t i = 0; i < particles.size(); i++) {
        ...
        std::array<double, 3> posFromVel =
                ArrayUtils::elementWiseScalarOp(deltaT, particle.getV(), std::multiplies<>());
        std::array<double, 3> acc = ArrayUtils::elementWiseScalarOp((1.0 / (2 * particle.getM())), particle.getF(),
                                                                    std::multiplies<>());
